...
#SAU
# calcul scoruri
scoruri = pca.getScoruri()
scoruri_df = pd.DataFrame(data=scoruri, index=numeObs,
                          columns=componente)
g.harta_intensitate(matrice=scoruri_df, titlu='Conponente principale standardizate (scoruri)')
# g.afisare()
##
...
##SAU
# calcul calitarea reprezentarii observatiilor
calObs = pca.getCalObs()
...
contribObs = pca.getContrib()
contribObs_df = pd.DataFrame(data=contribObs, index=numeObs, columns=componente)
g.harta_intensitate(matrice=contribObs_df, titlu='Cotributia observatiilor la varianta axelor')
# g.afisare()
##
